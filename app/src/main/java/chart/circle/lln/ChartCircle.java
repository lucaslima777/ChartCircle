...
public class ChartCircle extends View {
    ...
    public ChartCircle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initAttributes(@NonNull Context context, @Nullable AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            try {
                ...
                //BACKGROUND COLOR
                mBackgroundColor = a.getColor(R.styleable.ChartCircle_lln_backgroundColor, DEFAULT_BACKGROUND_COLOR);

                //BACKGROUND WIDTH
                mBackgroundWidth = a.getDimensionPixelSize(R.styleable.ChartCircle_lln_backgroundWidth, dp2px(DEFAULT_BACKGROUND_DP_WIDTH));

                //BACKGROUND FILL ENABLE STATE
                mFillBackground = a.getBoolean(R.styleable.ChartCircle_lln_fillBackground, false);

                //BACKGROUND FILL COLOR
                mBackgroundFillColor = a.getColor(R.styleable.ChartCircle_lln_fillBackgroundColor, DEFAULT_BACKGROUND_COLOR);

                //PROGRESS
                mPercentage = mTextPercentage = a.getInt(R.styleable.ChartCircle_lln_progress, 0);

                //PROGRESS COLOR
                mPercentageColor = a.getColor(R.styleable.ChartCircle_lln_percentageColor, DEFAULT_PERCENTAGE_COLOR);

                //PROGRESS WIDTH
                mPercentageWidth = a.getDimensionPixelSize(R.styleable.ChartCircle_lln_percentageWidth, dp2px(DEFAULT_PERCENTAGE_DP_WIDTH));

                //PROGRESS BAR STROKE STYLE
                int cap = a.getInt(R.styleable.ChartCircle_lln_percentageStyle, CAP_ROUND);
                percentageStyle = (cap == CAP_ROUND) ? Paint.Cap.ROUND : Paint.Cap.BUTT;

                //TEXT COLOR
                mTextColor = a.getColor(R.styleable.ChartCircle_lln_textColor, DEFAULT_PERCENTAGE_COLOR);

                //TEXT SIZE
                mTextSize = a.getDimensionPixelSize(R.styleable.ChartCircle_lln_textSize, sp2px(DEFAULT_TEXT_SP_SIZE));

                //TEXT TYPEFACE
                String typeface = a.getString(R.styleable.ChartCircle_lln_typeface);
                if (typeface != null && !typeface.isEmpty()) {
                    mTypeface = Typeface.createFromAsset(getResources().getAssets(), typeface);
                }

                //TEXT STYLE
                mTextStyle = a.getInt(R.styleable.ChartCircle_lln_textStyle, Typeface.NORMAL);
                if (mTextStyle > 0) {
                    if (mTypeface == null) {
                ...
                //START DRAWING ANGLE
                startAngle = a.getInt(R.styleable.ChartCircle_lln_startAngle, DEFAULT_START_ANGLE);
                if (startAngle < 0 || startAngle > 360) {
                    startAngle = DEFAULT_START_ANGLE;
                }

                //DRAWING ORIENTATION
                orientation = a.getInt(R.styleable.ChartCircle_lln_orientation, ORIENTATION_CLOCKWISE);

                //CHART MODE
                mode = a.getInt(R.styleable.ChartCircle_lln_mode, MODE_RING);

                //PROGRESS ANIMATION DURATION
                mAnimDuration = a.getInt(R.styleable.ChartCircle_lln_animDuration, DEFAULT_ANIMATION_DURATION);

                //PROGRESS ANIMATION INTERPOLATOR
                int interpolator = a.getInt(R.styleable.ChartCircle_lln_animInterpolator, DEFAULT_ANIMATION_INTERPOLATOR);
                switch (interpolator) {
                    case LINEAR:
